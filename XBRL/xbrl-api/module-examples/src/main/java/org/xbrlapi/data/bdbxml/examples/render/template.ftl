  ...
  <body>
		...
		<div style="display: none">
		  <ix:header>
            ...
            [#assign text = store.serialize(store.getSubtree(fragment)) /]
    	  ...
    	  <tr>
            ...
            [#list table.periods as period]
              <th class="value" colspan="1" rowspan="1">
                <div class="normal">
                  [#if ! period.missing]
                    ${period.id}
                    ${table.factSet.aspectModel.getLabeller(period.aspectId).getAspectValueLabel(period,"en-US",null,null)}
                  [#else]
                    missing period
                  [/#if]
                </div>
    	    ...
    	    [#if table.concepts?size > 0]
	            [#list 0..(table.concepts?size-1) as cIndex]
	              ...
	              <!-- Select facts matching this concept and this period. -->
	              [#assign conceptAspectValue = table.conceptAspect.getValue(concept) /]
	              
	              [#list table.periods as period]
	
                  [#assign items = table.factSet.getFacts([conceptAspectValue, period]) /]
