	    ...
	    try {
	        ...
	        for (UsedOn fragment: fragments) {
	            if (fragment.getParent().getDataRootElement().getAttribute("id").equals("newExtendedRoleType")) {
                    AssertJUnit.assertEquals(Constants.XBRL21LinkNamespace,fragment.getUsedOnNamespace());
                    AssertJUnit.assertEquals("presentationLink", fragment.getUsedOnLocalname());
                    //store.serialize(fragment);
                    AssertJUnit.assertTrue(fragment.isUsedOn(Constants.XBRL21LinkNamespace, "presentationLink"));
	            }
	        }
	    } catch (Exception e) {
