...
public class ResourceLanguageAttributeTestCase extends DOMLoadingTestCase {
    ...
    public void testGettingInheritedLanguageCodes() {
		...
		try {
			...
			AssertJUnit.assertEquals(3, resources.size());
			for (LabelResource resource: resources) {
			    logger.info(resource.serialize());
                AssertJUnit.assertNotNull(" No language code for label : " + resource.getStringValue(), resource.getLanguage());
			    logger.info(resource.getStringValue() + "(" + resource.getLanguage() + ")");
			    if (resource.getLanguage().equals("en")) {
			        AssertJUnit.assertEquals("English", resource.getStringValue());
			        foundLanguages++;
			    } else if (resource.getLanguage().equals("fr")) {
                    AssertJUnit.assertEquals("French", resource.getStringValue());
                    foundLanguages++;
			    } else if (resource.getLanguage().equals("de")) {
                    AssertJUnit.assertEquals("German", resource.getStringValue());
                    foundLanguages++;
			    } else Assert.fail(resource.getLanguage() + " is unexpected.");
			}
            AssertJUnit.assertEquals(3,foundLanguages);
		} catch (Exception e) {
			e.printStackTrace();
    ...
    public void testGettingInheritedLanguageCodesInSchemas() {
        ...
        try {
            ...
            AssertJUnit.assertEquals(1, concepts.size());

            for (Concept concept: concepts) {
                AssertJUnit.assertEquals("root", concept.getName());
                Assert.assertNotNull(concept.getLanguage());
                AssertJUnit.assertEquals("en", concept.getLanguage());
            }
            ...
            for (SimpleLink link: links) {
                if (link.getDataRootElement().getLocalName().equals("linkbaseRef")) {
                    AssertJUnit.assertEquals("fr", link.getLanguage());
                }
            }
            
            List<Schema> schemas = store.<Schema>getXMLResources(SchemaImpl.class);
            for (Schema schema: schemas) {
                if (schema.getTargetNamespace().equals("http://xbrlapi.org/test/xbrl/005")) {
                    ...
                    AssertJUnit.assertEquals(1, annotations.getLength());
                    for (int i=0; i< annotations.getLength(); i++) {
                        ...
                        AssertJUnit.assertEquals(1, appinfos.getLength());
                        for (int j=0; j < appinfos.getLength(); j++) {
                            Element appinfo = (Element) appinfos.item(j); 
                            AssertJUnit.assertEquals("fr", schema.getLanguage(appinfo));
                        }
                    }
