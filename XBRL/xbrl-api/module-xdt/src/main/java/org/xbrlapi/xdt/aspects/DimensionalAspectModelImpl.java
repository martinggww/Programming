                    ...
                    if (child.getNamespaceURI().equals(XDTConstants.XBRLDINamespace.toString())) {
                        ...
                        if (! dimensionQName.equals("")) {
                            ...
                            if (this.hasAspect(id)) {
                            	...
                            	Concept dimension = getStore().getConcept(candidateNamespace, candidateLocalname);
                            	if (dimension.getType().equals(ExplicitDimensionImpl.class.getName())) {
                                    ExplicitDimensionDomain domain = new ExplicitDimensionDomain(getStore(),candidateNamespace,candidateLocalname);
                                    Aspect aspect = new ExplicitDimensionAspect(domain,candidateNamespace,candidateLocalname);
                                    addAspect(aspect);
                            	} else if (dimension.getType().equals(TypedDimensionImpl.class.getName())) {
                            		TypedDimensionDomain domain = new TypedDimensionDomain(getStore(),candidateNamespace,candidateLocalname);
                                    Aspect aspect = new TypedDimensionAspect(domain,candidateNamespace,candidateLocalname);
                                    if (! hasAspect(aspect.getId())) addAspect(aspect);                            		
                            	}
